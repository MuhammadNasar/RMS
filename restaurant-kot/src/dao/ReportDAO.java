...
public class ReportDAO {
    ...
    public void printCreditCardReport(Vector<Report> vectorCreditCardReport, String toDate, String fromDate) {
        reportService = new ReportService();
            Vector<Report> vectorOfTotalCash = reportService.getOfTotalCreditCardVector(fromDate, toDate);
        try {
            PdfWriter writer = new PdfWriter("./from " + fromDate + " To " + toDate + " Sales On Credit-Card Report.pdf");
            PdfDocument pdfDocument = new PdfDocument(writer);
            Document layoutDocument = new Document(pdfDocument);
            pdfDocument.setDefaultPageSize(PageSize.A4);
            PdfPage pdfPage = pdfDocument.addNewPage();
            ...
            File pdfFile = new File("./from " + fromDate + " To " + toDate + " Sales On Credit-Card Report.pdf");
            if (pdfFile.exists()) {
                if (Desktop.isDesktopSupported()) {
                    Desktop.getDesktop().open(pdfFile);
                    System.out.println("File Has Been Opened Successfully!");
    ...
    public void printChequeReport(Vector<Report> vectorChequeReport, String toDate, String fromDate) {
        ...
        try {
            ...
            File pdfFile = new File("./from " + fromDate + " To " + toDate + " Sales On Cheque Report.pdf");
            if (pdfFile.exists()) {
                if (Desktop.isDesktopSupported()) {
                    Desktop.getDesktop().open(pdfFile);
                    System.out.println("File Has Been Opened Successfully!");
